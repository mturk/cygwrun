...
rv="`$_cygwrun -p I=\"\\\"./tmp\"\\\"`"
test "x$rv" = "xI=\".\\tmp\"" || xbexit 1 "Failed #3.10: \`$rv'"
rv="`$_cygwrun -p I=\"\\\"./tmp:./bar\"\\\"`"
test "x$rv" = "xI=\".\\tmp;.\\bar\"" || xbexit 1 "Failed #3.11: \`$rv'"
...
test "x$rv" = "x-f1=..\\tmp\\foo" || xbexit 1 "Failed #4.1: \`$rv'"
rv="`$_cygwrun $_dumpargs --f=../tmp/foo`"
test "x$rv" = "x--f=..\\tmp\\foo" || xbexit 1 "Failed #4.2: \`$rv'"
rv="`$_cygwrun $_dumpargs f=../tmp/foo`"
test "x$rv" = "xf=..\\tmp\\foo" || xbexit 1 "Failed #4.3: \`$rv'"

export FOO="/usr/a::/usr/b:"
rv="`$_cygwrun $_dumpenvp FOO`"
test "x$rv" = "xFOO=$usrdir\\a;$usrdir\\b" || xbexit 1 "Failed #5.1: \`$rv'"
export FOO="/usr:/bin: :../d"
export BAR="/usr/bin"
rv="`$_cygwrun $_dumpenvp FOO`"
test "x$rv" = "xFOO=$usrdir;$bindir;..\\d" || xbexit 1 "Failed #5.2: \`$rv'"
rv="`$_cygwrun -n FOO $_dumpenvp FOO`"
test "x$rv" = "xFOO=$FOO" || xbexit 1 "Failed #5.3: \`$rv'"
rv="`$_cygwrun -n \" FOO  ,,\" $_dumpenvp FOO`"
test "x$rv" = "xFOO=$FOO" || xbexit 1 "Failed #5.4: \`$rv'"
rv="`$_cygwrun -n \"FOO , ,    BAR\" $_dumpenvp BAR`"
test "x$rv" = "xBAR=$BAR" || xbexit 1 "Failed #5.5: \`$rv'"

rv="`$_cygwrun $srcdir/test/echoargs.bat /tmp /usr/a | tr -d '\r'`"
test "x$rv" = "x$tmpdir $usrdir\\a" || xbexit 1 "Failed #6.1: \`$rv'"
rv="`$_cygwrun $srcdir/test/echoargs.bat \"/tmp/some location\" | tr -d '\r'`"
test "x$rv" = "x\"$tmpdir\\some location\"" || xbexit 1 "Failed #6.2: \`$rv'"
